                ...
                {isExpanded && (
                  ...
                  >
                    <div className="border-t border-slate-100 bg-slate-50/60 px-5 py-4">
                      ...
                      {/* Pending Analysis CTA */}
                      {allDone && !g.ai_evidence_report && g.status === 'In Progress' && (
                        <motion.div
                          initial={{ opacity: 0, y: 8 }}
                          animate={{ opacity: 1, y: 0 }}
                          className="mt-5 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl flex items-center justify-between gap-4"
                        >
                          <div>
                            <p className="text-sm font-bold text-amber-800">⏳ AI Analyzing Evidence...</p>
                            <p className="text-xs text-amber-700 mt-0.5">
                              The AI Evidence Agent is currently verifying your resolution.
                            </p>
                          </div>
                          <button
                            onClick={() => navigate("/officer/evidence-report")}
                            className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-bold shadow hover:bg-amber-600 transition-colors flex-shrink-0"
                          >
                            <Bot size={15} /> View Status
                          </button>
                        </motion.div>
                      )}
                        ...
                        >
                          <div>
                            <p className="text-sm font-bold text-emerald-800">
                               {g.ai_evidence_report ? "⚠️ Resolution requires better evidence" : "🎉 All tasks complete!"}
                            </p>
                            <p className="text-xs text-emerald-700 mt-0.5">
                               {g.ai_evidence_report ? "The AI did not approve the resolution. Upload new before & after photos to retry." : "Upload before & after photos so the AI can verify the resolution."}
                            </p>
                          </div>
                          <div className="flex gap-2">
                            {g.ai_evidence_report && (
                              <button
                                onClick={() => navigate("/officer/evidence-report")}
                                className="flex items-center gap-2 px-4 py-2 bg-white text-emerald-700 border border-emerald-200 rounded-xl text-sm font-bold shadow-sm hover:bg-emerald-50 transition-colors flex-shrink-0"
                              >
                                View Report
                              </button>
                            )}
                            ...
                            >
                              <Upload size={15} /> {g.ai_evidence_report ? "Resubmit Evidence" : "Submit Evidence"}
                            </button>
                          </div>
                        </motion.div>
                      )}
