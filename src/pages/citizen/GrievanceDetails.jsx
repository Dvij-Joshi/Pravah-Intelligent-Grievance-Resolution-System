...
import {
  ...
  Zap, Bell, Camera, MessageSquare,
  TrendingUp, Activity, RefreshCw, AlertOctagon,
  Building2, CalendarDays, Hash, Loader2, ListChecks,
} from "lucide-react";
...
function buildGrievanceDetails(data, gid) {
  ...
  let tasks = aiTasks.map((t, i) => {
    ...
    return {
      id: t.id ?? i, title: t.title,
      assignee: t.department || "Field Team",
      status,
      time: `${Math.round(slaHours * ((i + 1) / Math.max(1, aiTasks.length)))}h deadline`,
    };
  });
  if (tasks.length === 0) tasks = [{ id: 0, title: "Waiting for AI Action Plan...", assignee: "AI Agent", status: "pending", time: "--" }];

  const updates = [{ id: 1, time: created.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), icon: "submit", msg: "Grievance submitted. ID: " + gid }];
  ...
  return {
    ...
    triage: data?.ai_triage_data || { summary: "Waiting...", urgency_reason: [], affected_population: "Unknown", confidence: 0 },
    steps, tasks, updates,
  };
}
        ...
        {steps.map((s, i) => (
          <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
            ...
            <div className={`relative z-10 w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all ${
              ...
              s.active ? "bg-blue-500 border-blue-400 shadow-md shadow-blue-200 animate-pulse" : "bg-white border-slate-300"
            }`}>
              {s.done ? <CheckCircle2 className="h-3.5 w-3.5 text-white" /> :
               s.active ? <span className="w-2 h-2 bg-white rounded-full" /> :
               <Circle className="h-3 w-3 text-slate-300" />}
            </div>
            <div className="flex-1 flex items-center justify-between">
              <span className={`text-sm font-semibold ${s.done ? "text-slate-700" : s.active ? "text-blue-700" : "text-slate-400"}`}>{s.label}</span>
              {s.active && <span className="text-xs font-bold text-blue-600 bg-blue-100 border border-blue-200 px-2 py-0.5 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />Active</span>}
...
function SlaRing({ pct }) {
  ...
  const r = 30; const circ = 2 * Math.PI * r; const dash = circ - (pct / 100) * circ;
  return (
    <svg width="80" height="80" viewBox="0 0 80 80">
      <circle cx="40" cy="40" r={r} fill="none" stroke="#e2e8f0" strokeWidth="7" />
      <motion.circle cx="40" cy="40" r={r} fill="none" stroke={color} strokeWidth="7" strokeDasharray={circ} strokeLinecap="round" strokeDashoffset={dash}
        initial={{ strokeDashoffset: circ }} animate={{ strokeDashoffset: dash }} transition={{ duration: 1, delay: 0.3 }} transform="rotate(-90 40 40)" />
...
export default function GrievanceDetails() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "grievances", filter: `id=eq.${id}` },
        (payload) => { setGrievance(payload.new); })
      .subscribe();
  ...
  const statusColor = g.status === "Resolved" ? "bg-green-100 text-green-700 border-green-200"
                    : g.status === "Overdue"   ? "bg-red-100 text-red-700 border-red-200"
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => navigate(-1)} className="p-2 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-4">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 mt-0.5">Your grievance has been marked as resolved. Please verify if the issue is actually fixed.</p>
              </div>
            </div>
            <button onClick={() => navigate(`/feedback/${id}`)}
              className="flex-shrink-0 flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-all duration-200 shadow-sm whitespace-nowrap">
          ...
          <div className="p-6">
            <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={`text-xs font-bold px-3 py-1 rounded-full border ${PRIORITY_BADGE[g.priority] || PRIORITY_BADGE.MEDIUM}`}>{g.priority}</span>
                <span className={`text-xs font-bold px-3 py-1 rounded-full border flex items-center gap-1 ${statusColor}`}>
                  <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse" />{g.status}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5 text-sm">
              {[
                ...
                { Icon: User,         label: "Assigned To",val: g.officer.name },
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
              ...
              {g.filesCount > 0 && (
                <div className="mt-2 flex items-center gap-1.5 text-xs text-blue-600 font-semibold">
                  <Camera className="h-3.5 w-3.5" />{g.filesCount} evidence file{g.filesCount > 1 ? "s" : ""} attached
                </div>
              )}
            </div>
            ...
            <div className="mt-5 flex items-center gap-5 bg-slate-50 border border-slate-100 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">{g.hoursElapsed}h elapsed of {g.slaHours}h</p>
                <div className="mt-2 h-2 bg-slate-200 rounded-full w-48 overflow-hidden">
                  <motion.div className={`h-full rounded-full ${g.slaPct >= 85 ? "bg-red-500" : g.slaPct >= 60 ? "bg-amber-500" : "bg-green-500"}`}
                    initial={{ width: 0 }} animate={{ width: `${g.slaPct}%` }} transition={{ duration: 1 }} />
                </div>
                <p className="text-xs text-slate-400 mt-1">{Math.max(0, g.slaHours - g.hoursElapsed)}h remaining</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          <AnimatePresence mode="wait">
            ...
            {/* ACTION PLAN TAB � merged Overview + tasks */}
            {tab === "plan" && (
              <motion.div key="plan" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
                {/* Resolution Journey */}
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  <h2 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                    <TrendingUp className="h-4 w-4 text-blue-600" /> Resolution Journey
                  </h2>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  <h2 className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-2">
                    <ListChecks className="h-4 w-4 text-blue-600" /> AI-Generated Action Steps
                  </h2>
                  <p className="text-xs text-slate-400 mb-5">Created by Resolution Planner Agent � Updated in real-time by officer</p>
                    ...
                    {g.tasks.map((task, i) => {
                      ...
                      return (
                        <motion.div key={task.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                          ...
                          <div className={`w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center ${s.dot} ${task.status === "active" ? "animate-pulse" : ""}`}>
                            {task.status === "done" ? <CheckCircle2 className="h-3.5 w-3.5 text-white" /> : <span className="text-white text-[9px] font-bold">{i + 1}</span>}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-semibold ${s.text} ${task.status === "done" ? "line-through opacity-70" : ""}`}>{task.title}</p>
                            <p className="text-xs text-slate-400">{task.assignee} � {task.time}</p>
                          </div>
                          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border flex-shrink-0 ${s.bg} ${s.text} ${s.border}`}>{task.status}</span>
                        </motion.div>
                      );
                    })}
                  </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  <h2 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                    <User className="h-4 w-4 text-blue-600" /> Assigned Officer
                  </h2>
                  <div className="flex items-center gap-4 bg-slate-50 border border-slate-200 rounded-xl p-4">
                    <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                      <span className="text-blue-700 font-bold text-lg">{g.officer.name.charAt(0)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-800">{g.officer.name}</p>
                      <p className="text-xs text-slate-500">{g.officer.role}</p>
            ...
            {tab === "updates" && (
              <motion.div key="updates" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                ...
                <h2 className="text-sm font-bold text-slate-700 mb-5 flex items-center gap-2">
                  <Bell className="h-4 w-4 text-blue-600" /> Activity Feed
                </h2>
                ...
                <div className="mt-6 border-t border-slate-100 pt-5">
                  ...
                  {feedbackSent ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      <CheckCircle2 className="h-4 w-4" /> Update request sent to officer
                    </motion.div>
                  ) : (
                  ...
                  <div className="bg-slate-950 rounded-xl p-4 font-mono text-xs space-y-0.5 overflow-x-auto">
                    ...
                    {grievance?.ai_triage_data ? (
                      Object.entries(grievance.ai_triage_data).map(([key, val], index, arr) => {
                        ...
                        return <p key={key}>&nbsp;&nbsp;<span className="text-blue-400">"{key}"</span>: {valSpan}{!isLast && ","}</p>;
                      })
                    ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="mt-2 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <motion.div className="h-full bg-green-500 rounded-full" initial={{ width: 0 }} animate={{ width: `${(g.triage?.confidence || 0.94) * 100}%` }} transition={{ duration: 1 }} />
                    </div>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 shadow-sm">
                    ...
                    <p className="text-xl font-extrabold text-amber-700">{grievance?.ai_triage_data?.recurrence_count ?? "�"} similar</p>
                    <p className="text-xs text-amber-600 mt-1">complaints nearby. AI monitoring for systemic patterns.</p>
